# Vendored, pinned reference copy of rtr46/meikiocr meikiocr/ocr.py
# at ebb8d2aedf69e62cbec57efb0bd00fb3e0e07297 (Apache-2.0; see THIRD_PARTY_NOTICES.md).
# Only change: models are loaded from a local directory instead of huggingface_hub,
# so parity fixtures are generated from exactly the lock-file bytes.
# ./meikiocr/ocr.py
...
def _get_model_path(repo_id, filename, model_dir=None):
    path = os.path.join(model_dir or os.environ.get("MEIKIOCR_MODEL_DIR", "models"), filename)
...
class MeikiOCR:
    def __init__(self, provider=None, max_batch_size=8, model_dir=None):
        """
        ...
        det_model_path = _get_model_path(DET_MODEL_REPO, DET_MODEL_NAME, model_dir)
        rec_model_path = _get_model_path(REC_MODEL_REPO, REC_MODEL_NAME, model_dir)
        vrec_model_path = _get_model_path(REC_MODEL_REPO, VREC_MODEL_NAME, model_dir)
